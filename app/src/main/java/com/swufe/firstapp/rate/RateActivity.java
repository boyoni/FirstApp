...
public class RateActivity extends AppCompatActivity implements Runnable {
        ...
        handler=new Handler() {//线程里的数据交给handle来处理
            public void handleMessage (Message msg){
                if (msg.what == 5) {
                    ...
                    Toast.makeText(RateActivity.this, "获取网络汇率", Toast.LENGTH_SHORT).show();
                }
    ...
    public void btn(View btn) {
        ...
        if (btn.getId() == R.id.button1){
            ...
            Toast.makeText(RateActivity.this, "美元换算成功！", Toast.LENGTH_SHORT).show();
        }else if (btn.getId() == R.id.button2) {
            float euro = rmb * euro_rate;
            text.setText(String.valueOf(euro) + "元");
            Toast.makeText(RateActivity.this, "欧元换算成功！", Toast.LENGTH_SHORT).show();
        } else if(btn.getId()==R.id.button3) {
            float won = rmb * won_rate;
            text.setText(String.valueOf(won) + "元");
            Toast.makeText(RateActivity.this, "韩元换算成功！", Toast.LENGTH_SHORT).show();
        }
    }

    //点击open按钮，打开新界面，将汇率传值到新的activity
    public void config(View v) {
        Intent newActvity = new Intent(this, ConfigActivity.class);//intent意图对象，是动作的抽象描述，保存要执行的操作
